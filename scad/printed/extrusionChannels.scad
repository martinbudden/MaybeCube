...
*/
    ...
    difference() {
        union() {
            ...
            //add the lip
            translate([-lipX/2, taperY]) square([lipX, lipY]);
        }
        // make a cut down the middle so the clip can be squeezed
        translate([-cutX/2, 0]) square([cutX, taperY+lipY]);
    }
}
module E2020Clip(size, channelWidth=6.0, channelDepth=1.9, lipOverhang=0.3) {
     linear_extrude(size) E2020Clip2d(channelWidth, channelDepth);
}
...
module _E2020Stripe(channelWidth, channelDepth, lipOverhang=0.3) {
    ...
    thickness = 1;
    difference() {
        translate([-stripeX/2, channelDepth]) square([stripeX, stripeY]);
        translate([-(stripeX-thickness*2)/2, channelDepth]) square([stripeX-thickness*2, stripeY]);
    }
}
...
module E2020Cover(length, topOnlyLength=0, channelWidth=6.0, channelDepth=1.8, coverSizeX=e2020CoverSizeX) {
    ...
    linear_extrude(length - topOnlyLength) {
        ...
        translate([0, e2020CoverSizeY]) E2020Clip2d(channelWidth, channelDepth);
    }
    translate([0, 0, length - topOnlyLength]) linear_extrude(topOnlyLength) {
        _E2020Base(coverSizeX, e2020CoverSizeY);
    }
}
