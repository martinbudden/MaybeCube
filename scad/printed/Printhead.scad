...
use <../../../BabyCube/scad/utils/ziptieCutout.scad>
...
function grooveMountSize(xCarriageType, hotend_type, blower_type) = [hotendOffset(xCarriageType, hotend_type).x + xCarriageBackSize(xCarriageType).x/2, 36, 12];
...
module E3Dv6plusFan() {
    ...
    translate_z(3)
        rotate(180)
            hot_end(E3Dv6, filament=1.75, naked=true, bowden=true);
}
...
module blowerTranslate(xCarriageType, hotend_type, blower_type, z=0) {
    ...
    translate([-grooveMountSize.x + z, blower_length(blower_type) - 16 + grooveMountOffsetX(hotend_type), -blower_size(blower_type).x - grooveMountClampSize(xCarriageType, hotend_type, blower_type).y/2])
        translate([hotendOffset.x, xCarriageBackOffsetY(xCarriageType)+18, hotendOffset.z])
            rotate([90, 0, -90])
                children();
}
