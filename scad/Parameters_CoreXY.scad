...
function leftDrivePulleyOffset() = useReversedBelts() ? [26, use2060ForTopRear() ? -7 : -6] : [useXYDirectDrive ? 0 : 38 + 3*largePulleyOffset, -largePulleyOffsetTop];
function rightDrivePulleyOffset() = [useXYDirectDrive ? 0 : -42.5 - 3*largePulleyOffset, useReversedBelts() ? (use2060ForTopRear()? -7 : -6) : -largePulleyOffsetTop]; // need to give clearance to extruder motor
