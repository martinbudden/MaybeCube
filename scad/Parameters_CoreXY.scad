...
function leftDrivePulleyOffset() = useReversedBelts() ? [30, use2060ForTopRear() ? -14 : -6] : [useXYDirectDrive ? 0 : 38 + 3*largePulleyOffset, -largePulleyOffsetTop];
function rightDrivePulleyOffset() = [useXYDirectDrive ? 0 : -42.5 - 3*largePulleyOffset, useReversedBelts() ? (use2060ForTopRear()? -14 : -6) : -largePulleyOffsetTop]; // need to give clearance to extruder motor
function plainIdlerPulleyOffset() = useReversedBelts() ? [0, use2060ForTopRear() ? -24 : -20] : largePulleyOffset ? [3, -3] : [0, 0];
